#!/usr/bin/env python
# coding: utf-8
import sys
from scipy.io import loadmat
import matplotlib
import numpy as np
import matplotlib.pyplot as plt
from vmdpy import VMD
# matplotlib.use('TkAgg')
plt.rcParams["figure.figsize"]=(12,5)
print('matplotlib backend:',matplotlib.get_backend())
'''
own
'''
T = 1/125 #采样周期
fs = 1/T  #sample frequency
# t = np.arange(1,T+1)/T
t = np.arange(0,8,T)
# freqs=np.linspace(0,int(fs/2),len(t)/2)
freqs=np.arange(0,1000)*fs/1000
print('t shape {}，freqs shape {}'.format(t.shape,freqs.shape))
T=1000
'''
# Time Domain 0 to T
T = 1000  #采样周期
fs = 1/T   #采样频率
t = np.arange(1,T+1)/T
# t = np.arange(1,8)/T
freqs = 2*np.pi*(t-0.5-fs)/(fs)
'''


# center frequencies of components
f_1 = 2
f_2 = 24
f_3 = 288

# modes
v_1 = (np.cos(2*np.pi*f_1*t))
v_2 = 1/4*(np.cos(2*np.pi*f_2*t))
v_3 = 1/16*(np.cos(2*np.pi*f_3*t))

#% for visualization purposes
fsub = {1:v_1,2:v_2,3:v_3}
wsub = {1:2*np.pi*f_1,2:2*np.pi*f_2,3:2*np.pi*f_3}

# composite signal, including noise

f = v_1 + v_2 + v_3 + 0.1*np.random.randn(v_1.size)

'''
change input to ppg signal 
'''
data=loadmat('/home/wcj/CurrentProject/segan_pytorch_ppg/NewData/noisy_trainset/3-8-95.2381.mat')
f=data['ytmp1'][0]
print('f shape:{},f type:{}'.format(f.shape,type(f)))
print('len f:{}'.format(len(f)))

plt.figure()
plt.plot(f)
plt.title('original signal')


# f_hat = np.fft.fftshift((np.fft.fft(f)))
f_hat = np.fft.fft(f)
print('f_hat shape:',f_hat.shape)

# some sample parameters for VMD
alpha = 457   # moderate bandwidth constraint
tau = 0.155    # noise-tolerance (no strict fidelity enforcement)
K = 7              # 3 modes
DC = 0           # no DC part imposed
init = 1           # initialize omegas uniformly
tol = 1e-8


# Run actual VMD code
"""
input:
    signal and some parameters
output: 
    u:the collection of decomposed modes 分解模式的集合
    u_hat:spectra of the modes 模式的光谱
    omega:estimated mode center-frequencies 估计模式中心频率
"""
u, u_hat, omega = VMD(f, alpha, tau, K, DC, init, tol)
print('type u:{},type u_hat:{} type omega {}'.format(type(u),type(u_hat),type(omega)))
print('u shape:{},u_hat shape:{},omege shape:{}'.format(u.shape,u_hat.shape,omega.shape))

# Simple Visualization of decomposed modes
plt.figure()
plt.plot(u.T)
plt.title('Decomposed modes')

# For convenience here: Order omegas increasingly and reindex u/u_hat
sortIndex = np.argsort(omega[-1,:])
omega = omega[:,sortIndex]
u_hat = u_hat[:,sortIndex]
u = u[sortIndex,:]
linestyles = ['b', 'g', 'm', 'c', 'c', 'r', 'k']

# fig1 = plt.figure()
# plt.subplot(411)
# plt.plot(t,f)
# plt.xlim((0,1))
# for key, value in fsub.items():
#     plt.subplot(4,1,key+1)
#     plt.plot(t,value)
# fig1.suptitle('Original input signal and its components')

fig2 = plt.figure()
# plt.loglog(freqs[T//2:], abs(f_hat[T//2:]))
plt.plot(freqs[:T//2], abs(f_hat[:T//2]))
# plt.xlim(np.array([1,T/2])*np.pi*2)
ax = plt.gca()
ax.grid(which='major', axis='both', linestyle='--')
fig2.suptitle('Input signal spectrum')

fig3 = plt.figure()
print('omega shape:',omega.shape)
# sys.exit(0)
for k in range(K):
#     plt.semilogx(2*np.pi/fs*omega[:,k], np.arange(1,omega.shape[0]+1), linestyles[k])
    plt.plot(fs*omega[:,k], np.arange(1,omega.shape[0]+1), linestyles[k])
fig3.suptitle('Evolution of center frequencies omega')

fig4 = plt.figure()
# plt.loglog(freqs[T//2:], abs(f_hat[T//2:]), 'k:')
plt.plot(freqs[:T//2], abs(f_hat[:T//2]), 'k:')
# plt.xlim(np.array([1, T//2])*np.pi*2)
for k in range(K):
#     plt.loglog(freqs[T//2:], abs(u_hat[T//2:,k]), linestyles[k])
    plt.plot(freqs[:T//2], abs(u_hat[T//2:,k]), linestyles[k])
fig4.suptitle('Spectral decomposition')
plt.legend(['Original','1st component','2nd component','3rd component','4rd component','5rd component','6rd component','7rd component'])

fig4 = plt.figure()
for k in range(K):
    plt.subplot(7,1,k+1)
    plt.plot(t,u[k,:], linestyles[k])
    # plt.plot(t, fsub[k+1], 'k:')
    plt.xlim((0,8))
    plt.title('Reconstructed mode %d'%(k+1))
plt.show(block=True)   
# plt.show()  










